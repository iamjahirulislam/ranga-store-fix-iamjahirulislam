...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product shadow rounded m-2">
      <div>
    <img class="product-image mb-3" src=${image}></img>
      ...
      <p class = "d-none" id="current-product-id"> product id: ${product.id} </p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now  customise-btn">Add to cart</button>
      <button id="details-btn" class="customise-btn">Details</button></div>
      `;
...
// updated delivery charge and total tax function
